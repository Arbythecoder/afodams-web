...
const protect = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) {
        try {
            ...
            // Get user from token (using id which matches what we sign in the token)
...
const landlordOrAdmin = (req, res, next) => {
    if (req.user && (req.user.role === "landlord" || req.user.role === "admin")) {
        ...
        res.status(403).json({ message: "Not authorized as landlord or admin" });
    }
};
...
module.exports = { protect, admin, landlordOrAdmin, authorize };